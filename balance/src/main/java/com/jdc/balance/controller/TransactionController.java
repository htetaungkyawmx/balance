...
@WebServlet({ "/employee/transaction/search", "/employee/transaction/edit", "/employee/transaction/details",
...
public class TransactionController extends BaseController {
	...
	private void search(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		var list = transacionService().search(code, type, from, to, category);
	...
	private void showDetails(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		var data = transacionService().findById(Integer.parseInt(idParam));
	...
	private void edit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (null != idParam && !"0".equals(idParam)) {
			data = transacionService().findById(Integer.parseInt(idParam));
		}
		...
		if (!isPostRequest(req)) {
			...
			navigate(destinationBuilder(EDIT_VIEW, Type.Income.equals(data.getType())).req(req).resp(resp).build());
		} else {
			...
			data = transacionService().save(data);
	...
	private void approve(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		var idParam = req.getParameter("id");
		transacionService().approve(Integer.parseInt(idParam));
